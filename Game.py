...
class Game:
    ...
    def EndGameText(self):
        ...
        text_1 = 'Player ' + str(winner) + ' has won the game with ' + str(player_point[winner-1]) + ' points!'
    ...
    def check_valid_move(self, event):
        ...
        if 0 < cur < 12 and self.table.table[cur] != [0,0]:
            if (self.turn_step <= 1 and 6 < cur < 12) or (self.turn_step > 1 and 0 < cur < 6):
    ...
    def play(self, event):
        if self.turn_step == 1 or self.turn_step == 3:
            ...
            direction = abs(playerID - (event == pygame.K_LEFT))
    ...
    def game_over(self):
        if self.table.table[0] == [0,0] and self.table.table[6] == [0,0]:
            global player_point
            player_point[0] += sum(self.table.table[i][0] for i in range(1, 6))
            player_point[1] += sum(self.table.table[i][0] for i in range(7, 12))
            return True
        return False